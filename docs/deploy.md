...
本项目已部署在 `openclaw` 服务器上，通过 Systemd 进行服务托管。

---

## 1. 托管环境信息

- **服务器 SSH 别名**：`openclaw` (配置在本地 `~/.ssh/config` 中)
  - IP 地址：`104.168.117.208`
  - SSH 端口：`2221`
  - SSH 用户：`root`
- **服务器项目工作目录**：`/opt/website-collect-bot`
- **运行环境**：`.venv` (Python 虚拟环境)
...
4. SSH 连接服务器 `openclaw` 执行 `git pull` 拉取最新代码。
